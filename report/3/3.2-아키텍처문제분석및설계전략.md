...
| AS-10 | 외부 연계 의존성 캡슐화 | ISSUE-08 | AD-09, AD-10 |

아래 표는 아키텍처 이슈별 설계 문제, 드라이버, 선정 전략을 요약한 것이다.

| 아키텍처 설계문제 | 아키텍처 드라이버 | 아키텍처 설계전략 | 설계전략 선정이유 |
| :---: | ----- | ----- | ----- |
| ISSUE-01. 동시 입장 시 스레드·커넥션 풀 고갈 | AD-02. 2만 명 동시 입장 처리 성능 | AS-02. 입장 처리 경로 비동기 전환 | Meeting Manager Feign 동기 호출 구간에서 서블릿 스레드를 즉시 반환하여 2만 건 동시 요청 스레드 고갈 방지 |
...
| ISSUE-03. 입장 요청과 단순 조회 처리 우선순위 미분리 | AD-02. 2만 명 동시 입장 처리 성능 | AS-04. 입장 전용 처리 경로 확보 | 입장 전용 포트(8081) 분리로 단순 조회와 스레드 경쟁 구조적으로 차단 |
...
|  |  | AS-03. 외부 권한 조회 다층 캐시 적용 | 캐시 히트 시 외부 서버 호출 자체를 제거하여 연쇄 지연 원천 차단 |
...
|  |  | AS-10. 외부 연계 의존성 캡슐화 | ACL 패턴으로 외부 모델 변경이 포털 도메인에 직접 전파되지 않도록 격리 |
| ISSUE-09. 예약 회의 기반 선제 대응 구조 부재 | AD-01. 피크 시 권한 갱신 응답시간<br>AD-02. 2만 명 동시 입장 처리 성능<br>AD-04. 핵심 기능 성공률 99.9% | AS-03. 외부 권한 조회 다층 캐시 적용 | L2 Redis 캐시 인프라를 피크 전 선제 적재의 저장소로 활용 |
...
AS-01 (도메인 경계 분리)
  ...
  └── AS-10 (의존성 캡슐화) — AS-01의 외부 연계 모듈 분리를 구현하는 패턴
...
| AD-06 2만 명 동시 회의 입장 | AS-02 (비동기 전환) + AS-04 (전용 경로 확보) + AS-08 (격벽 분리) |
...
이 구조에서 파생되는 아키텍처 문제는 세 가지다. 첫째, 도메인 경계가 없으므로 기능별 HikariCP 커넥션 풀을 분리(AS-08 격벽 분리)하더라도 어떤 DB 접근이 어느 도메인에 속하는지 명확히 구분할 수 없다. 둘째, 참석자 상태 Command(입장·퇴장 write)와 Query(목록 조회 read)가 동일 도메인 모델을 공유하므로 DB 경로 분리(AS-07) 적용 시 경계 기준이 모호하다. 셋째, WC서버·VC서버·AC서버·Copilot Admin 서버 등 10개 이상의 외부 연계 로직이 동일 서비스 레이어에 혼재하여(ISSUE-08), 특정 연계만 긴급 차단하거나 타임아웃을 조정하려면 yml 수정 후 어플리케이션 재구동이 필요하여 트래픽 집중 중 런타임 긴급 조정이 불가능하다.

**대안**

**대안 1. 현행 구조 유지**

front-api / server-api / admin-api로 배포 단위는 분리되어 있으나, 코드 레벨에서 도메인 경계가 없다. 회의 입장 처리, 권한 갱신, 10개 이상의 외부 연계 로직이 동일 서비스 레이어에 혼재하며, AS-08 격벽 분리를 적용하려 해도 어떤 DB 접근이 어느 도메인에 속하는지 구분할 기준이 없다. ISSUE-04·ISSUE-08이 구조적으로 해소되지 않고, 파생 전략의 전제 조건을 충족할 수 없다.
...
```
<p align="center"><em>[그림 9] 대안1 — 현행 구조 유지</em></p>

**대안 2. 완전한 마이크로서비스 분리**

서비스·DB·배포 단위를 도메인별로 완전히 독립시킨다. 코드 경계가 물리적으로 명확해지는 장점이 있으나, 기존 front-api와 server-api 간 DB 트랜잭션 경계를 Saga 패턴 등 분산 트랜잭션으로 재설계해야 한다. 메시지 브로커 등 신규 인프라 도입이 수반되어 C-04(점진적 적용) 제약을 위반하며, 분산 트랜잭션 설계 복잡도가 즉시 부상한다.
...
단일 코드베이스와 기존 배포 구조를 유지하면서 `domain.entry`, `domain.auth`, `domain.meeting`, `integration.wc`, `integration.ac`, `integration.copilot` 등 패키지 경계를 코드 레벨에서 명시적으로 설정한다. 도메인 간 참조는 인터페이스만 허용하고 ArchUnit으로 빌드 타임에 강제한다. 기술 스택 변경 없이 AS-08·AS-07·AS-10 파생 전략의 전제 조건을 충족한다.
...
| 대안 | 개념 | 한계 |
| :---: | ----- | ----- |
| 대안 1. 현행 구조 유지 | URI prefix로 역할 분리, 코드 레벨 도메인 경계 없음 | AS-08·AS-10 파생 전략의 전제 조건 충족 불가. ISSUE-04·08이 구조적으로 해소되지 않음 |
...
- `integration.*` 패키지에 외부 서버별 Feign Client + AS-09 CB + AS-10 의존성 캡슐화 변환 캡슐화
...
```mermaid
flowchart LR
    subgraph BEFORE["As-is — 도메인 경계 없음"]
        direction TB
        SVC["서비스 레이어\n(입장·권한·회의·외부연계 혼재)"]
        DB_B[("MariaDB\n단일 커넥션 풀 공유")]
        SVC --> DB_B
        NOTE_B["⚠ 기능별 커넥션 풀 분리 기준 없음\n⚠ 외부 연계 정책 독립 관리 불가"]
    end

    subgraph AFTER["To-Be — 도메인 모듈 경계 분리"]
        direction TB
        ...
        subgraph INTEGRATION["integration.*  (AS-10 ACL)"]
            I1["integration.wc"]
            I2["integration.ac"]
            I3["integration.copilot"]
        end
        DB_A[("MariaDB\n기능별 독립 풀 — AS-08 Bulkhead")]
        DOMAIN --> INTEGRATION
        D1 --> DB_A
        D2 --> DB_A
        D3 --> DB_A
    ...
    style NOTE_B fill:#fff3cd,color:#000
    style D1 fill:#d4edda,color:#000
    style I1 fill:#cce5ff,color:#000
    style I2 fill:#cce5ff,color:#000
    style I3 fill:#cce5ff,color:#000
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as01-domain-boundary.png -->
<p align="center"><em>[그림 12] 도메인 경계 부재(As-is)와 패키지 모듈 경계 분리(To-Be) 비교</em></p>

**파생 전략**: AS-08 (격벽 분리), AS-07 (DB 경로 분리), AS-10 (의존성 캡슐화)
...
ISSUE-01의 핵심 병목은 Meeting Manager Feign 동기 호출 구간이다. "DB 입장 가능 여부 확인 → conference-token 발급" 단계까지는 포털 서버 내부에서 빠르게 처리된다. 그러나 "Meeting Manager에 참석자 입장 정보 조회(Feign 동기, 3,000ms)"가 완료될 때까지 해당 요청의 서블릿 스레드가 블로킹된다. MM 응답값을 받아야 wyzProParam을 조립하고 클라이언트에 응답할 수 있으므로 이 호출은 생략할 수 없다. 2만 건이 동시에 이 단계에 도달하면 2만 개의 스레드가 Meeting Manager 응답을 대기하는 상태가 된다. 해결의 핵심은 **외부 서버 호출 구간에서 서블릿 스레드를 즉시 반환**시키는 것이다.

**대안**

**대안 1. 현행 Feign 동기 호출 유지**

`maxThreads` 증가로 처리 용량을 간접 확대하는 방식이다. 2만 건이 동시에 Meeting Manager Feign 호출 단계에 도달하면 2만 개 서블릿 스레드가 최대 3,000ms 블로킹되는 구조적 문제가 해소되지 않는다. 스레드 수 증가는 컨텍스트 스위칭 오버헤드와 JVM 힙 압박을 증가시키며, QA-02 달성의 구조적 한계를 극복하지 못한다.
...
Meeting Manager Feign 호출 구간만 `@Async("externalCallExecutor")`로 선택적 비동기화하고, Spring MVC와 HikariCP를 그대로 유지한다. 컨트롤러가 `CompletableFuture<ResponseEntity>`를 반환하면 Spring MVC가 서블릿 스레드를 즉시 반환하고, externalCallExecutor에서 Meeting Manager 조회 + wyzProParam 조립이 완료된 후 응답을 전송한다. 2만 건 동시 요청에서도 서블릿 스레드 풀 고갈이 발생하지 않는다.
...
```mermaid
sequenceDiagram
    participant C as 클라이언트 (2만 명)
    participant ST as 서블릿 스레드 풀
    participant EX as externalCallExecutor
    participant MM as Meeting Manager

    Note over C,ST: As-is — 서블릿 스레드 블로킹
    C->>+ST: 입장 요청
    ST->>+MM: Feign 동기 호출 (최대 3,000ms 블로킹)
    Note over ST: ⚠ 스레드 점유 대기
    MM-->>-ST: 응답
    ST-->>-C: 반환

    Note over C,MM: To-Be — @Async 비동기 전환
    C->>+ST: 입장 요청
    ST--)EX: @Async("externalCallExecutor") 위임
    ST-->>C: 서블릿 스레드 즉시 반환
    EX->>+MM: 비동기 호출
    MM-->>-EX: 응답
    EX-->>C: CompletableFuture 완료 → 응답 전송
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as02-async-flow.png -->
<p align="center"><em>[그림 16] Feign 동기 호출(As-is)과 @Async 비동기 전환(To-Be) 비교</em></p>
...
flowchart TD
    ...
    PREWARM["AS-06 선제 초기화\n피크 N분 전 선제 적재"]
    PREWARM -->|선제 적재| L2

    style PREWARM fill:#d4edda,color:#000
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as03-cache-flow.png -->
<p align="center"><em>[그림 17] L1(Caffeine) · L2(Redis) 계층 캐시 조회 흐름</em></p>

**대안**

**대안 1. 캐시 없음 (현행)**

매 로그인 시 AC서버·Copilot Admin 서버에 비동기 병렬 호출 후 `CompletableFuture.allOf()`로 모든 응답을 대기한다. 변경 빈도가 낮은 권한 데이터임에도 매 로그인마다 불필요한 외부 호출이 반복되고, 피크 시간대 외부 서버 응답 지연이 로그인 응답 시간에 직접 전이된다. QA-01 달성이 외부 서버 응답 속도에 전적으로 종속된다.
...
인스턴스 로컬 L1 Caffeine(TTL 5분)과 분산 공유 L2 Redis(TTL 30분~1시간)를 계층적으로 구성한다. L1 히트 시 네트워크 없이 즉시 반환, L2 히트 시 외부 서버 호출 없이 반환하여 다중 인스턴스 환경에서도 외부 서버 호출을 일괄 완충한다. L2 Redis 인프라가 AS-06 선제 초기화의 실질적 기반이 된다.
...
**채택 근거**: 대안 2는 인스턴스 간 캐시 공유 불가, DB 커넥션 추가 소비, cold start 미해소 등의 한계가 있다. 대안 3은 인스턴스 스케일아웃 환경에서도 외부 서버 호출을 일괄 완충하며, AS-06 선제 초기화의 실질적 기반이 된다.
...
<p align="center"><em>[그림 21] Tomcat Connector 포트 분리 — 입장 전용(8081)과 일반(8080) 구분</em></p>
...
flowchart TD
    ...
    subgraph FILTER["ThrottlingInterceptor (front-api)"]
        EXEMPT{"@ThrottleExempt\n어노테이션?"}
        BUCKET["Bucket4j\nSlidingWindowCounter\n비핵심 API 상한: 초당 500 req"]
        PASS["정상 처리"]
    ...
    PEAK -->|피크 비활성| PASS
    PEAK -->|피크 활성| EXEMPT
    EXEMPT -->|면제 대상\n(/join · /conference-token · 회의 시작)| PASS
    EXEMPT -->|비핵심 API\n(단순 조회 · 통계 등)| BUCKET
    BUCKET -->|상한 이내| PASS
    BUCKET -->|상한 초과| REJECT

    style PASS fill:#d4edda,color:#000
    style REJECT fill:#ff9999,color:#000
    style BUCKET fill:#fff3cd,color:#000
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as05-throttling.png -->
<p align="center"><em>[그림 25] 피크 구간 감지 기반 차등 스로틀링 흐름 — 핵심 API 면제, 비핵심 API 선별 제한</em></p>
...
```mermaid
sequenceDiagram
    participant SCH as PreWarmingScheduler<br/>(1분 주기, preWarmExecutor)
    ...
    participant PD as PeakDetector<br/>(AS-05 Throttling)

    Note over SCH: 피크 N분 전 감지
    SCH->>DB: 시작 시각 임박 + 참석자 수 ≥ 임계값 회의 조회
    DB-->>SCH: 대상 회의 목록 + 참석자 목록
    SCH->>PD: setActive(true) — AS-05 Throttling 활성화
    loop 50명/배치 (100ms 간격)
        SCH->>EXT: 참석자 권한 선제 조회
        EXT-->>SCH: 권한 데이터
        SCH->>L2: L2 Redis 선제 적재
    end
    Note over L2: 피크 진입 시 캐시 히트율 높은 상태<br/>→ Thundering Herd 방지
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as06-prewarm.png -->
<p align="center"><em>[그림 29] 예약 회의 기반 L2 Redis 선제 적재 흐름 — 피크 N분 전 실행</em></p>

**대안**

**대안 1. 현행 피동 대응**

트래픽이 실제로 집중된 이후 자연적 warm-up에 의존한다. 피크 초입에 대량의 캐시 미스가 일시에 외부 서버로 집중되는 "Thundering Herd" 현상이 발생한다. AS-03 캐시를 도입하더라도 피크 진입 시점에 캐시가 비어 있으면 이 문제는 해소되지 않는다.
...
```
<p align="center"><em>[그림 30] 대안1 — 현행 피동 대응</em></p>
...
<p align="center"><em>[그림 33] Primary(write) · Replica(read) 분리 라우팅 구조</em></p>

**대안**

**대안 1. 현행 선택적 Replica 라우팅 유지**

Primary-Replica 인프라는 갖추어져 있으나, 응답 지연이 심한 일부 SELECT 쿼리에 한해 개별 수동으로 Replica를 지정하는 방식이다. 적용 기준이 일관되지 않아 트랜잭션 단위 readOnly 경계를 보장하지 못하고 누락 위험이 있다. 가장 근본적으로, ISSUE-07의 핵심인 write lock 경합(셀프 참석자 front-api INSERT · cPaaS 피드백 경유 server-api UPDATE가 동일 Primary에 집중)은 SELECT를 Replica로 옮겨도 해소되지 않는다.
...
모든 Command를 이벤트 스트림으로 저장하고 Query는 별도 Read Model(투영)을 조회한다. Command와 Query 경로가 완전히 분리되는 이상적인 구조이나, 기존 JPA 엔티티 기반 도메인 모델을 이벤트 소싱 모델로 전면 재설계해야 한다. 이벤트 순서 보장·투영 복구·이벤트 버저닝 등 복잡한 문제를 수반하며 C-04(점진적 적용) 제약을 위반한다.
...
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as09-bulkhead.png -->
<p align="center"><em>[그림 37] HikariCP 커넥션 풀 — 단일 풀(As-is)과 도메인별 Bulkhead(To-Be) 비교</em></p>

**대안**

**대안 1. 현행 단일 HikariCP 풀**

`maximumPoolSize` 증가로 커넥션 고갈을 지연시키는 방식이다. 풀 크기를 늘려도 입장 처리가 풀 전체를 소진하면 회의 조회·회의 개설 등 다른 기능의 DB 접근이 차단된다. QA-03(입장 풀 고갈 시 회의 시작 API 성공률 100%)은 단일 풀 구조에서 원칙적으로 충족 불가능하다.
...
| query-pool | Read 전용 (Replica, AS-07) | 30 | 3,000ms |
...
외부 서버들은 특성이 서로 다르다. WC서버는 회의 개설·종료(UC-03, UC-07) 처리에 필수적이고, Copilot Admin 서버(LLM·용어사전 권한)는 장애 시 DB 저장값으로 폴백 가능하다. 외부 서버별로 timeout·실패율 임계값·fallback 전략이 달라야 하는 이유다.
...
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as10-circuit-breaker.png -->
<p align="center"><em>[그림 41] CB 상태 전환(Closed · Open · Half-Open)과 외부 서버별 계층적 Fallback 경로</em></p>

**대안**

**대안 1. 현행 Feign + Hystrix CB 유지**

서킷 브레이커가 이미 적용되어 있으나, 전역 단일 설정으로 모든 외부 서버에 동일 임계값이 적용된다. WC서버(회의 개설·종료 필수, 빠른 감지 필요)와 Copilot Admin 서버(DB 폴백 가능, 관대한 임계값 허용)에 동일 정책이 적용되어 서버 특성에 맞는 정밀 제어가 불가능하다. 또한 Hystrix는 Spring Boot 3.x에서 공식 지원이 종료된 deprecated 라이브러리다.
...
- AS-10 ACL의 각 연계 모듈 내에 `@CircuitBreaker(name = "wcServer", fallbackMethod = "wcFallback")` 어노테이션 적용
...
```mermaid
flowchart LR
    subgraph BEFORE["As-is — 서비스 레이어 직접 연계"]
        direction TB
        SVC_B["MeetingJoinService\nMeetingService\nAuthService"]
        FC_B["WcServerFeignClient\nVcServerFeignClient\nAcServerFeignClient"]
        SVC_B -->|직접 의존| FC_B
        NOTE_B["⚠ 외부 서버 스키마 변경 → 서비스 레이어 전파\n⚠ 연계 정책 변경 → 전체 재배포 강제"]
    end

    subgraph AFTER["To-Be — integration.* ACL 캡슐화"]
        direction TB
        subgraph DOMAIN_A["domain.*"]
            SVC_A["MeetingJoinService\n(WcServerGateway 인터페이스만 의존)"]
        end
        subgraph INT_A["integration.wc"]
            GW["WcServerGateway\n(인터페이스)"]
            ADAPT["WcServerAdapter\n(DTO 변환)"]
            CB_A["WcCircuitBreakerConfig\n(CB 정책 독립 캡슐화)"]
            FEIGN["WcServerFeignClient"]
        end
        EXT_A["WC서버"]
        SVC_A --> GW
        GW --> ADAPT
    ...
    style CB_A fill:#cce5ff,color:#000
```
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/3.2-as10-acl-module.png -->
<p align="center"><em>[그림 45] 서비스 레이어 직접 연계(As-is)와 ACL 캡슐화 모듈 구조(To-Be) 비교</em></p>

**대안**

**대안 1. 현행 서비스 레이어 직접 연계**

`MeetingJoinService`가 `WcServerFeignClient`를, `MeetingService`가 `VcServerFeignClient`·`AcServerFeignClient`를 직접 의존한다. 외부 서버 API 스키마 변경이 서비스 레이어까지 직접 전파되고, WC서버 타임아웃 조정이나 AC서버 재시도 정책 변경처럼 특정 연계만 독립적으로 수정·배포하는 것이 불가능하다.
...
```
<p align="center"><em>[그림 46] 대안1 — 현행 서비스 레이어 직접 연계</em></p>

**대안 2. 외부 서버별 Adapter 인터페이스 추상화**

포털 도메인 인터페이스를 정의하고 Feign Client 기반 구현체를 별도로 작성한다. 도메인 모델이 외부 서버 스키마에서 격리되는 효과는 있으나, CB 정책·Fallback 전략·타임아웃 설정이 여전히 분산되어 있다. 연계별 독립 정책 관리 구조가 만들어지지 않아 재배포 강제 문제가 해소되지 않는다.
...
```
<p align="center"><em>[그림 47] 대안2 — 외부 서버별 Adapter 인터페이스 추상화</em></p>

**대안 3. 연계별 독립 모듈 ACL (정책 캡슐화 + 도메인 격리) (채택)**

AS-01의 `integration.*` 패키지 구조를 기반으로 각 외부 연계를 `Gateway 인터페이스 + Adapter + CircuitBreakerConfig + FallbackHandler`가 포함된 독립 모듈로 캡슐화한다. 도메인 레이어(`domain.*`)는 `integration.*`의 인터페이스만 의존하며, 외부 서버 스키마 DTO는 `integration.*` 내부에만 존재한다. 연계별 CB 정책과 Fallback 전략을 다른 연계 영향 없이 독립적으로 변경할 수 있다.
...
```
<p align="center"><em>[그림 48] 대안3 — 연계별 독립 모듈 ACL (채택)</em></p>
...
| 대안 2. 외부 서버별 Adapter 인터페이스 추상화 | 포털 도메인 인터페이스 정의 + Feign Client 기반 구현체 별도 작성 | 변환 레이어(Adapter)는 생기지만 연계별 독립 정책 관리 구조는 없음. 어플리케이션 재구동 강제 |
