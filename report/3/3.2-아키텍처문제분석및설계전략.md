...
AS-07 (선제 초기화)
  └── AS-06 (유입 제한)      — 선제 초기화 스케줄러가 피크 임박을 감지하면 유입 제한 동시 활성화
```
...
이 구조에서 파생되는 아키텍처 문제는 세 가지다. 첫째, 도메인 경계가 없으므로 기능별 HikariCP 커넥션 풀을 분리(AS-09 격벽 분리)하더라도 어떤 DB 접근이 어느 도메인에 속하는지 명확히 구분할 수 없다. 둘째, 참석자 상태 Command(입장·퇴장 write)와 Query(목록 조회 read)가 동일 도메인 모델을 공유하므로 DB 경로 분리(AS-08) 적용 시 경계 기준이 모호하다. 셋째, WC서버·VC서버·AC서버·Copilot Admin 서버 등 10개 이상의 외부 연계 로직이 동일 서비스 레이어에 혼재하여(ISSUE-08), 특정 연계만 긴급 차단하거나 타임아웃을 조정하려면 전체 재배포가 강제된다.

**대안**

**대안 1. 현행 구조 유지**

front-api / server-api / admin-api로 배포 단위는 분리되어 있으나, 코드 레벨에서 도메인 경계가 없다. 회의 입장 처리, 권한 갱신, 10개 이상의 외부 연계 로직이 동일 서비스 레이어에 혼재하며, AS-09 격벽 분리를 적용하려 해도 어떤 DB 접근이 어느 도메인에 속하는지 구분할 기준이 없다. ISSUE-04·ISSUE-08이 구조적으로 해소되지 않고, 파생 전략의 전제 조건을 충족할 수 없다.
...
단일 코드베이스와 기존 배포 구조를 유지하면서 `domain.entry`, `domain.auth`, `domain.meeting`, `integration.wc`, `integration.ac`, `integration.copilot` 등 패키지 경계를 코드 레벨에서 명시적으로 설정한다. 도메인 간 참조는 인터페이스만 허용하고 ArchUnit으로 빌드 타임에 강제한다. 기술 스택 변경 없이 AS-09·AS-08·AS-11 파생 전략의 전제 조건을 충족한다.
...
- `integration.*` 패키지에 외부 서버별 Feign Client + AS-10 CB + AS-11 의존성 캡슐화 변환 캡슐화
- `domain.entry` 전용 `DataSource` Bean 설정 → AS-09 격벽 분리 기반 마련

**파생 전략**: AS-09 (격벽 분리), AS-08 (DB 경로 분리), AS-11 (의존성 캡슐화)
...
ISSUE-01의 핵심 병목은 Meeting Manager Feign 동기 호출 구간이다. "DB 입장 가능 여부 확인 → conference-token 발급 → 입장 파라미터 생성" 단계까지는 포털 서버 내부에서 빠르게 처리된다. 그러나 "Meeting Manager에 입장 파라미터 전달(Feign 동기, 3,000ms)"이 완료될 때까지 해당 요청의 서블릿 스레드가 블로킹된다. 2만 건이 동시에 이 단계에 도달하면 2만 개의 스레드가 Meeting Manager 응답을 대기하는 상태가 된다. 해결의 핵심은 **외부 서버 호출 구간에서 서블릿 스레드를 즉시 반환**시키는 것이다.
...
Meeting Manager Feign 호출 구간만 `@Async("externalCallExecutor")`로 선택적 비동기화하고, Spring MVC와 HikariCP를 그대로 유지한다. 서블릿 스레드는 외부 서버 호출을 비동기 executor에 위임하고 즉시 반환되므로, 2만 건 동시 요청에서도 서블릿 스레드 풀 고갈이 발생하지 않는다. AS-05 조기 반환의 기반이 된다.
...
- Meeting Manager 입장 파라미터 전달 메서드, VC/AC 서버 회의 개설 호출 메서드에 `@Async` 적용
- `CompletableFuture` 반환 타입 적용으로 AS-05 조기 반환에서 결과 추적 가능
- 비동기 실패 처리는 AS-10 외부 서버 장애 차단과 연동하여 fallback 처리
...
또한 ISSUE-09에서 지적된 cold start 문제는, 캐시가 존재하더라도 피크 진입 시점에 캐시가 비어 있으면 해소되지 않는다. 따라서 캐시 인프라 자체가 AS-07(선제 초기화)의 선제 워밍 기반이 되어야 한다.
...
인스턴스 로컬 L1 Caffeine(TTL 5분)과 분산 공유 L2 Redis(TTL 30분~1시간)를 계층적으로 구성한다. L1 히트 시 네트워크 없이 즉시 반환, L2 히트 시 외부 서버 호출 없이 반환하여 다중 인스턴스 환경에서도 외부 서버 호출을 일괄 완충한다. L2 Redis 인프라가 AS-07 선제 초기화의 실질적 기반이 된다.
...
**채택 근거**: 대안 2는 인스턴스 간 캐시 공유 불가, DB 커넥션 추가 소비, cold start 미해소 등의 한계가 있다. 대안 3은 인스턴스 스케일아웃 환경에서도 외부 서버 호출을 일괄 완충하며, AS-07 선제 초기화의 실질적 기반이 된다.
...
- AS-09(격벽 분리)와 결합 시: 입장 전용 Connector 스레드는 `join-pool` HikariCP DataSource만 사용하도록 구성
...
①~③은 포털 서버 내부에서 처리되어 빠르다. ④에서 Meeting Manager Feign 동기 호출이 완료되어야 ⑤를 반환할 수 있는 구조가 병목이다. 핵심 질문은 **"⑤를 반환하기 위해 ④가 반드시 완료되어야 하는가?"**이다. Meeting Manager 전달이 미완료 상태에서 PC 클라이언트가 입장을 시도하면 cPaaS 세션이 아직 준비되지 않은 상태일 수 있으나, 이 간격을 재시도 메커니즘으로 흡수할 수 있다.

**대안**

**대안 1. 현행 동기 응답**

Meeting Manager에 입장 파라미터 전달이 완료된 후 클라이언트에 응답을 반환한다. 2만 건 동시 입장 시 2만 개 서블릿 스레드가 최대 3,000ms 동안 Meeting Manager 응답을 대기하므로, QA-02 달성이 Meeting Manager의 응답 속도에 전적으로 종속된다. AS-02가 적용되더라도 이 패턴을 유지하면 비동기 처리의 서블릿 스레드 즉시 반환 효과를 살리지 못한다.

**대안 2. 폴링 방식**

입장 처리 ID를 즉시 반환하고 클라이언트가 주기적으로 결과를 폴링한다. 서블릿 스레드 점유를 단축할 수 있으나, 기존 클라이언트(웹·모바일·PC)의 API 호출 방식을 변경해야 하므로 C-02(기존 API 하위 호환성 유지) 제약을 위반한다.

**대안 3. 즉시 응답 + 비동기 후처리 (채택)**

①~③ 단계(DB 확인, conference-token 발급, 입장 파라미터 생성)가 완료되는 시점에 즉시 클라이언트에 응답을 반환하고, ④ Meeting Manager 전달은 `@Async externalCallExecutor`에서 후처리한다. 기존 API 스키마를 유지하면서 서블릿 스레드 점유 시간을 외부 서버 호출 시간(최대 3,000ms)에서 포털 내부 처리 시간(수십~수백ms)으로 단축한다.

**대안 비교**

| 대안 | 개념 | 한계 |
|------|------|------|
| 대안 1. 현행 동기 응답 | Meeting Manager 응답 완료 후 클라이언트 반환 | 2만 건 동시 입장 시 2만 개 서블릿 스레드 블로킹. QA-02 달성이 Meeting Manager 응답 속도에 종속 |
...
**채택 근거**: 대안 1은 QA-02 달성이 Meeting Manager 응답 속도에 종속. 대안 2는 C-02 위반으로 선택 불가. 대안 3은 기존 API 스키마를 유지하면서 서블릿 스레드 점유 시간을 외부 서버 호출 시간(최대 3,000ms)에서 포털 내부 처리 시간(수십~수백ms)으로 단축한다.
...
AS-04(입장 전용 처리 경로 확보)가 "핵심 요청이 비핵심 요청보다 먼저 처리되도록" 하는 전략이라면, AS-06은 "피크 구간에 비핵심 요청의 유입량 자체를 줄여" 핵심 처리 경로의 리소스 여유를 확보하는 전략이다. 두 전략은 보완 관계다. AS-07(선제 초기화)이 예약 데이터를 활용해 캐시를 선제 적재한다면, AS-06은 동일한 피크 예측 정보를 활용해 **비핵심 요청의 유입을 시간 구간 기반으로 제어**한다.
...
- AS-09(기능별 격벽 분리)와 결합: Primary DataSource는 `join-pool`·`service-pool`, Replica DataSource는 `query-pool`로 별도 HikariCP 풀 설정

---

#### AS-09. 기능별 커넥션·스레드 격벽 분리
...
WC서버·VC서버·AC서버·Copilot Admin 서버의 역할과 장애 허용 범위에 따라 `slidingWindowSize`, `failureRateThreshold`, `waitDuration`을 독립적으로 설정한다. 각 서버별로 역할에 맞는 Fallback 전략(AS-05 재시도 큐, DB 폴백, Redis→DB 계층 폴백)을 연계하여, 단일 외부 서버 장애가 포털 전체 가용성에 미치는 영향을 최소화한다.
