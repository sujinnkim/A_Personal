...
**적용 방향**
- `externalCallExecutor`: corePoolSize 200, maxPoolSize 800, queueCapacity 4,000으로 설정
...
현행 서비스에는 ConcurrentHashMap 기반 로컬 캐시가 이미 활용되고 있다. 그러나 이는 TTL 설정이 없어 권한 데이터가 변경되어도 캐시가 자동 만료되지 않으며, 크기 제한·Eviction 정책이 없으므로 8만 명 환경에서 메모리가 무제한 증가할 수 있다. 캐시 통계(hit율·miss율)를 수집할 수 없어 QA-01 달성 여부를 정량적으로 관측하는 수단도 없다. 인스턴스 간 캐시 공유 역시 불가능하므로 front-api 스케일아웃 시 인스턴스마다 외부 서버 호출이 중복 발생한다. AS-03은 이 네 가지 구조적 한계를 해소하기 위한 전략이다.
...
**대안 1. ConcurrentHashMap 로컬 캐시 유지 (현행)**

현행 ConcurrentHashMap 기반 로컬 캐시 구조를 그대로 유지한다. 외부 서버에서 권한을 조회한 결과를 JVM 로컬 ConcurrentHashMap에 저장하여 동일 사용자의 반복 요청에 재사용한다.

TTL이 없어 권한 변경이 캐시에 자동 반영되지 않으며, 크기 제한·Eviction 정책이 없어 8만 명 환경에서 메모리가 무제한 증가한다. 캐시 통계 수집이 불가능하고, JVM 로컬 한정이므로 인스턴스 간 공유가 되지 않아 스케일아웃 시 중복 외부 호출이 발생한다.

```mermaid
flowchart LR
    REQ["로그인 요청"] --> CHM{"ConcurrentHashMap\n(TTL 없음 · 크기 제한 없음)"}
    CHM -->|"hit"| RESP["응답 반환"]
    CHM -->|"miss"| AC["AC서버 호출"]
    CHM -->|"miss"| CP["Copilot Admin 호출"]
    AC & CP -->|"allOf() 대기"| RESP
    style CHM fill:#cccccc,color:#000
```
<p align="center"><em>[그림 18] 대안1 — ConcurrentHashMap 로컬 캐시 유지 (현행)</em></p>
...
| 대안 | 개념 | 한계 |
| :---: | ----- | ----- |
| 대안 1. ConcurrentHashMap 유지 (현행) | JVM 로컬 ConcurrentHashMap 캐시 유지 | TTL·Eviction·통계 없음. 메모리 무제한 증가 위험. 인스턴스 간 공유 불가. 권한 변경 자동 반영 안 됨 |
...
**채택 근거**: 대안 1(ConcurrentHashMap 유지)은 TTL·Eviction·통계가 없어 권한 데이터 신선도 보장, 메모리 제어, QA-01 관측이 구조적으로 불가능하다. 대안 2(DB 캐시)는 인스턴스 간 캐시 공유 불가, DB 커넥션 추가 소비, cold start 미해소 등의 한계가 있다. 대안 3은 Caffeine으로 TTL·Eviction·통계를 확보하고, L2 Redis로 인스턴스 간 공유와 AS-06 선제 초기화 기반을 동시에 마련한다. 기존 ConcurrentHashMap 코드를 CacheManager 설정 교체만으로 전환 가능하여 C-04(점진적 적용) 제약도 준수한다.
...
- 입장 전용 Connector: `maxThreads=400`, `minSpareThreads=100`
- 일반 Connector(8080): `maxThreads=400`
...
flowchart TD
    ...
    PEAK -->|피크 비활성| PASS
    PEAK -->|피크 활성| EXEMPT
    EXEMPT -->|"면제 대상 /join · /conference-token · 회의 시작"| PASS
    EXEMPT -->|"비핵심 API 단순 조회 · 통계 등"| BUCKET
    BUCKET -->|상한 이내| PASS
    BUCKET -->|상한 초과| REJECT
...
- Bucket4j `SlidingWindowCounter`: 비핵심 API 전역 처리량 상한 (피크 구간 중 초당 2,000 req)
...
AS-03(캐시)이 도입되면, 캐시가 채워진 상태에서는 외부 서버 호출 없이 빠른 응답이 가능하다. 그러나 캐시가 도입되더라도 **피크 진입 시점에 캐시가 비어 있으면** (TTL 만료, 서버 재시작, 신규 사용자 등) 피크 초입의 대량 캐시 miss가 일시에 외부 서버로 쏟아지는 "Thundering Herd" 현상이 발생한다. Pre-warming은 이 역설을 깨는 전략이다. 트래픽이 실제로 집중되기 N분 전에, DB의 예약 회의 데이터를 조회하여 해당 회의 참석자들의 권한 캐시를 선제적으로 L2 Redis에 적재한다.
...
sequenceDiagram
    ...
    Note over L2: 피크 진입 시 캐시 hit율 높은 상태<br/>→ Thundering Herd 방지
```
...
| ----- | ----- | ----- | ----- |
| join-pool | 입장 처리 전용 | 200 | 3,000ms |
| service-pool | 회의 시작·초대 | 60 | 5,000ms |
| general-pool | 권한 갱신·일반 조회 | 120 | 5,000ms |
| query-pool | Read 전용 (Replica, AS-07) | 120 | 3,000ms |
...
ISSUE-06의 핵심 메커니즘: WC서버가 장애 상태에 빠지면 Hystrix CB가 개방되기 전까지 Feign 호출은 read timeout(3,000ms) 만료까지 블로킹된다. 8만 명 동시 입장 구간에 WC서버 장애가 겹치면, CB 개방 이전 구간에만 `externalCallExecutor` 스레드 풀에 3,000ms씩 블로킹된 스레드가 빠르게 누적된다.
