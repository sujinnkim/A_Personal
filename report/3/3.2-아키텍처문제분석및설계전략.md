...
기존 도메인 패키지 구조를 유지하면서 `domain.entry` 전용 DataSource Bean을 분리 설정하여 AS-08 Bulkhead의 기술적 전제 조건을 마련한다. 도메인 간 참조는 인터페이스만 허용하도록 ArchUnit으로 빌드 타임에 강제하여 경계 침식을 방지한다. 기술 스택 변경 없이 AS-08·AS-07·AS-10 파생 전략의 전제 조건을 충족한다.
...
**채택 근거**: 완전 MSA(대안 2)는 분산 트랜잭션 설계와 신규 인프라 구성이 수반되어 C-04(점진적 적용) 제약을 위반한다. 대안 3은 배포 구조와 기술 스택을 변경하지 않으면서 DataSource Bean 분리와 ArchUnit 규칙으로 AS-08·AS-07·AS-10 파생 전략의 전제 조건을 충족한다.
...
```mermaid
flowchart LR
    subgraph BEFORE["As-is — 패키지 분리됨, DataSource 공유"]
        direction TB
        subgraph PKGS["패키지 분리 (컨벤션)"]
            D_B["domain.*\n(conference·auth·member)"]
            A_B["api.*\n(meetingmanager·copilot·ac)"]
        end
        DS_B["단일 HikariCP DataSource\n(모든 도메인 공유)"]
        DB_B[("MariaDB")]
        PKGS --> DS_B --> DB_B
        NOTE_B["⚠ DataSource 단일 공유로\n기능별 커넥션 풀 귀속 기준 없음"]
    end
...
현행 코드에는 외부 연계 서버별 패키지(api/meetingmanager, api/copilot, api/ac 등)에 Feign Configuration과 Fallback 구현이 이미 존재한다. 그러나 Java 코드 수준에서 SetterFactory를 재정의하지 않고 application.yml에 전역 기본값만 설정되어 있어, 서버별 차등 CB 임계값이 실질적으로 적용되지 않는 상태다.
...
현행 코드는 외부 연계 서버별로 패키지(api/meetingmanager, api/ac 등)가 분리되어 있으며, Feign Client·Fallback·Configuration·Decoder가 서버별로 구현되어 있다. 그러나 도메인 서비스 레이어(`MeetingJoinService`, `AuthService` 등)는 이 Feign Client 인터페이스를 직접 의존한다. 이 구조의 문제는 두 가지다.
...
도메인 서비스 레이어가 서버별 Feign Client 인터페이스를 직접 의존한다. 외부 서버 API 스키마 변경이 Feign Client DTO를 넘어 서비스 레이어까지 전파되고, 특정 연계 타임아웃·재시도 정책만 독립적으로 수정·배포하는 것이 불가능하다.
