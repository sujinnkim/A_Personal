...
미팅(Meeting)은 사내 임직원과 대외 고객사를 대상으로 제공되는 영상 회의 솔루션이다. 웹, 모바일, PC 클라이언트, 회의실 장비(VC 단말), 전화 참석(오디오 컨퍼런스) 등 다양한 단말과 참석 방식을 지원하며, 사용자들이 일상적인 업무 회의에 활용한다.

최근에는 다양한 AI 기능이 접목되어, 회의 종료 후 대화 내용을 AI로 요약한 회의록 제공, 자동 언어 인식 기반의 실시간 번역 서비스, 회의 중 AI 코파일럿 기능 등을 사용할 수 있다.

서비스는 현재 대외 고객사 및 공공기관으로의 확대를 진행 중이며, 특정 대규모 사업 부문(약 8만 명 규모의 임직원)을 대상으로 하는 대규모 스트리밍 서비스 오픈이 예정되어 있다. 부문 행사 등에서 전 임직원이 동시 참석하는 경우 최대 동시 접속 규모가 8만 명에 이를 수 있으며, 이 값이 본 과제에서 상정하는 최대 동시 입장 규모의 근거다. 이에 따라 특정 시간대에 대규모 동시 접속과 트래픽 집중이 발생할 것으로 예측된다.
...
<p align="center"><em>[표 1] 미팅 포털 서버 기능 영역</em></p>

> *wyzProParam*: 회의 입장 시 클라이언트에 전달되어야 하는 사용자의 권한 정보와 회의 관련 정보. front-api가 Meeting Manager 조회 응답값과 포털 서버 내부 값들을 조합해 생성하며, 웹/모바일을 경유해 런처로 전달된다.
...
<p align="center"><em>[표 2] URI prefix별 API 인스턴스 및 역할</em></p>
...
![As-is 구조도](..\..\report\images\1.1-asis-architecture.png)
<!-- 이미지 파일명(draw.io → PNG 교체 시): report/images/1.1-asis-architecture.png -->
<p align="center"><em>[그림 1] AS-IS 미팅 포털 서버 아키텍처 구조도</em></p>

회의 생성 흐름은 front-api의 전처리 후 Meeting Manager로 진입한다. front-api는 단순 Gateway가 아니며, 미팅 타입 결정·권한 확인·유효성 검사·전처리를 직접 수행한 뒤 Meeting Manager에 회의 개설을 요청한다. Meeting Manager는 WC 채널 설정(cPaaS 경유)을 처리하고, VC/AC가 포함된 경우 개설 요청을 server-api에 전달한다.
...
사용자 요청 외에 백엔드 발생 역방향 흐름도 존재한다. 회의 중 참석자 상태 변경(퇴장, 연결 끊김 등)은 `cPaaS → server-api → DB`로 처리되며(피드백 흐름), front-api를 거치지 않는다.
...
<p align="center"><em>[표 3] 미팅 타입 코드 및 명칭</em></p>
...
<p align="center"><em>[표 4] 배포 환경 및 대상</em></p>
...
**트래픽 집중 구간의 처리 한계**
업무 시작 시간대(오전 9시, 오후 1시 등)에는 다수의 사용자가 동시에 로그인하고 회의에 입장하는 패턴이 반복된다. 8만 명 규모 스트리밍 서비스 오픈 시 이 집중도는 더욱 심화될 것으로 예상된다.
