...
입장 처리 도메인의 클래스 구성·결합·핵심 다이어그램을 다룬다. 본 패키지는 UC-04 회의 입장과 conference-token 발급을 단독 책임지며, 요청 집중 구간의 핵심 병목 경로다. 핵심 관심사는 AS-08 join-pool 격벽과 AS-04 입장 전용 Connector(8081)로, 입장 처리가 다른 기능과 스레드·커넥션을 다투지 않도록 격리하는 데 있다.
