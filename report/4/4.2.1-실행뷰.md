...
| P1~P4 | join/service/general/query 풀 | 기능별 HikariCP 격벽 | AS-08 |
...
외부 서버 장애 시 CB Open 전환과 서버별 차등 fallback 흐름이다. 필수 서버(Meeting Manager)의 fail-fast(그림 51a)와 폴백 가능 서버(Copilot Admin)의 계층적 복구(그림 51b)로 나눈다.
...
| 51b-③④ | copilotAdmin CB Open → L2 폴백 | AS-09 + AS-03 | Copilot Admin 장애 시 캐시 기반 계층 복구 |
| 51b-⑥ | DB 저장값 최종 폴백 | AS-09 | L2 miss 시 DB 저장 권한값으로 서비스 연속성 유지 |
