...
- **선별적 격리 불가**: ISSUE-06처럼 특정 외부 서버 장애 시 해당 연계만 차단·폴백 처리하거나, 연계별로 타임아웃·재시도 정책을 달리 적용하는 세밀한 제어가 불가능하다. 단일 코드베이스에서는 한 연계의 정책 변경이 다른 연계에 영향을 줄 수 있어, 피크 대응을 위한 긴급 조정 시 전체 재배포가 강제된다.
