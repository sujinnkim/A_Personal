...
- 연계 시스템별 인증 방식, 타임아웃 정책, 재시도 로직은 Hystrix Feign 설정으로 개별 적용되어 있으나, 모두 단일 코드베이스에 산재해 있어 특정 연계 설정만 긴급 조정하려 해도 전체 서비스 재배포가 강제된다.
...
- **선별적 격리 불가**: 연계별 타임아웃·재시도 정책은 Hystrix Feign 설정으로 개별 적용되어 있으나, 단일 코드베이스에 모여 있어 특정 연계 설정을 긴급 조정하려면 전체 서비스 재배포를 거쳐야 한다. 한 연계의 설정 변경이 동일한 빌드·배포 사이클에 묶여 있어, 피크 대응 중 즉각적인 정책 조정이 사실상 불가능하다.
