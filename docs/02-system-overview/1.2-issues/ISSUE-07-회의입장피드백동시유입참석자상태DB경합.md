...
- **사용자 요청 경유**: `User → front-api → Meeting Manager → server-api → DB write` (회의 입장 등)
- **피드백 흐름 경유**: `cPaaS → Meeting Manager → server-api → DB write` (퇴장, 연결 끊김 등 참석자 상태 변경)

두 경로 모두 server-api를 통해 동일한 DB 테이블에 write를 수행하며, 피드백 흐름은 사용자 요청과 무관하게 백엔드에서 독립적으로 발생한다.

## 문제점

- Command(상태 변경)와 Query(조회)가 동일 DB 레코드에 대해 read/write lock을 경쟁하여 lock 경합이 발생한다.
- 대규모 미팅 시작 시점에 사용자 입장(write)과 cPaaS 피드백(write)이 동시에 집중되면, 단일 DB 모델의 lock 경합이 최고조에 달한다.
