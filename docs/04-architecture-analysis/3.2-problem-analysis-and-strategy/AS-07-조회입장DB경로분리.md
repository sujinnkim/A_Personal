...
*대안3 — @Transactional 기반 Primary/Replica 전체 체계화 (채택)*
...
- AS-08(기능별 커넥션·스레드 격벽 분리)와 결합: Primary DataSource는 `join-pool`·`service-pool`, Replica DataSource는 `query-pool`로 별도 HikariCP 풀 설정
