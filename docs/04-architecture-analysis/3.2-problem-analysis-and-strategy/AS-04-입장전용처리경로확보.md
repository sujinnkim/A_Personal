...
- **아키텍처 드라이버**: AD-02 (8만 명 동시 입장 처리 성능)
- **해결 이슈**:
...
*대안2 — URL 패턴 기반 전용 Connector·스레드 풀 분리 (채택)*
...
*대안3 — HandlerInterceptor + 인메모리 우선순위 큐 재정렬*
...
- 입장 전용 Connector: `maxThreads=200`, `minSpareThreads=50` (피크 시에도 최소 50스레드 보장)
- 일반 Connector(8080): `maxThreads=300` (단순 조회·권한 갱신 등)
- API Gateway 또는 Nginx에서 `/meetings/*/join`, `/meetings/*/conference-token` URL 패턴을 포트 8081로 라우팅
- AS-08(Bulkhead)와 결합 시: 입장 전용 Connector 스레드는 `join-pool` HikariCP DataSource만 사용하도록 구성
