...
**[외부 서버 호출 스레드 풀 격리]**
- 기존 Hystrix threadpool(deprecated)을 `@Async` 기반 `externalCallExecutor`로 교체 (AS-02에서 정의): 외부 서버(WC서버, VC서버, AC서버) Feign 호출 전담 (`corePoolSize=50`, `maxPoolSize=200`, `queueCapacity=1,000`)
