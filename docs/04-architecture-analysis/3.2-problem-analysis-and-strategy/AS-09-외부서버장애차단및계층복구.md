...
그런데 현재 시스템의 외부 서버들은 특성이 서로 다르다. WC서버는 회의 개설·종료(UC-03, UC-07) 처리에 필수적이고, Copilot Admin 서버(LLM·용어사전 권한)는 장애 시 DB 저장값으로 폴백 가능하다. AC서버 장애 시에는 AC 미포함 회의의 입장·시작은 계속 가능해야 한다. 외부 서버별로 timeout·실패율 임계값·fallback 전략이 달라야 하는 이유다.
