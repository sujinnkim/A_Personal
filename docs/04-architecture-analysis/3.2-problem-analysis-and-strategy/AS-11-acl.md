...
둘째, **연계별 독립 정책 관리가 불가능**하다. ISSUE-08에서 지적한 대로, WC서버 타임아웃을 1,500ms로 줄이거나 AC서버에 재시도 로직을 추가하려면, 해당 연계 설정 변경이 전체 코드베이스 빌드·배포 사이클을 거쳐야 한다. 피크 대응 시 긴급하게 특정 연계만 차단·폴백 설정을 변경하는 것이 구조적으로 어렵다.
