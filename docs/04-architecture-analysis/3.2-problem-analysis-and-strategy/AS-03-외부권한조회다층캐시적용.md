...
또한 ISSUE-09에서 지적된 cold start 문제는, 캐시가 존재하더라도 피크 진입 시점에 캐시가 비어 있으면 해소되지 않는다. 따라서 캐시 인프라 자체가 AS-06(예약 기반 피크 자원 선제 초기화)의 선제 워밍 기반이 되어야 한다.

## 대안

### 대안 1. ConcurrentHashMap 로컬 캐시 유지 (현행)

**개념**: 현행 ConcurrentHashMap 기반 로컬 캐시 구조를 그대로 유지한다. 외부 서버에서 권한을 조회한 결과를 JVM 로컬 ConcurrentHashMap에 저장하여 동일 사용자의 반복 요청에 재사용한다.

**이 시스템 적용 방식**: 변경 없음.

**한계**: 첫째, TTL이 없어 권한 데이터가 변경되어도 캐시가 자동 만료되지 않는다. 관리자가 권한을 변경해도 서버 재시작 전까지 이전 값이 반환될 수 있다. 둘째, 크기 제한·Eviction 정책이 없으므로 8만 명 환경에서 메모리가 무제한 증가한다. 셋째, 캐시 통계(hit율·miss율)를 수집할 수 없어 QA-01 달성 여부를 정량적으로 관측할 수단이 없다. 넷째, JVM 로컬에 한정되어 front-api 인스턴스 간 공유가 불가능하므로 스케일아웃 시 인스턴스마다 외부 서버 중복 호출이 발생한다.
...
*대안1 — ConcurrentHashMap 로컬 캐시 유지 (현행)*
...
**개념**: L1 로컬 JVM 캐시(Caffeine)와 L2 분산 캐시(Redis)를 계층화하여 적용한다. 캐시 miss 시 L1 → L2 → 외부 서버 순으로 폴백한다. 권한 유형별로 TTL을 차등 적용하여 변경 빈도에 맞게 캐시 신선도를 유지한다.
...
**장점**: 피크 시간대 외부 서버 요청을 L1·L2 캐시 hit로 대부분 흡수하여 외부 서버 부하를 대폭 감소시킨다. 인스턴스 수 증가 시에도 L2 Redis를 공유하므로 외부 서버 요청이 선형 증가하지 않는다. 또한 AS-06(예약 기반 피크 자원 선제 초기화)이 피크 전에 L2 Redis를 선제 적재하면 피크 초입 cold start 없이 캐시 hit율을 유지할 수 있다.
...
**채택 근거**: 대안 1(ConcurrentHashMap 유지)은 TTL·Eviction·통계가 없어 권한 데이터 신선도 보장, 메모리 제어, QA-01 관측이 구조적으로 불가능하다. 대안 2(DB 캐시)는 인스턴스 간 캐시 공유 불가, DB 커넥션 추가 소비, cold start 미해소 등의 한계가 있다. 대안 3은 Caffeine으로 TTL·Eviction·통계를 확보하고, L2 Redis로 인스턴스 간 공유와 AS-06 Pre-warming 기반을 동시에 마련한다. Spring `@Cacheable`과 Caffeine·Redis 통합은 기존 ConcurrentHashMap 코드를 CacheManager 설정 교체만으로 전환 가능하므로 C-04(점진적 적용) 제약도 준수한다.
...
**파생 전략**:
- AS-06 (Predictive Pre-warming): L2 Redis 캐시가 존재해야 피크 전 선제 적재(Pre-warming) 효과 발생
