...
- **입장 처리 순서**:
  1. DB에서 입장 가능 여부 확인 — 사용자가 해당 회의 참석자로 초대되어 있는지, 또는 회의 번호·URL로 누구나 참석 가능한 오픈 회의인지 확인한다.
  2. 확인이 통과되면 `/conference-token` API로 인미팅 전용 토큰을 발급한다.
  3. DB에서 사용자 권한을 조회하고 입장 파라미터(사용자 권한 정보 포함)를 생성한다.
  4. WC서버에 입장 파라미터를 Feign 동기 호출(read timeout 3,000ms)로 전달하고 응답을 수신한다.
  5. 입장 파라미터를 반환하면 PC 클라이언트 또는 모바일 클라이언트(인미팅)가 실행된다.
